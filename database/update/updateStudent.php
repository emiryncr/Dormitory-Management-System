<?php 
    ...
    $sql = "SELECT * FROM users WHERE userid = '$id' AND role = 'student'";
    ...
    if(isset($_POST["submit"])){
        ...
        $sql = "UPDATE users SET userid = '$up_id', username = '$up_username', name = '$up_name', surname = '$up_surname', email = '$up_email', phone = '$up_phone' WHERE userid = '$up_id'";

        if(mysqli_query($connection, $sql)){
            header('Location: ../../mainPages/admin/admin.php?page=manage-students');
        }else{
            echo "Error while updating the student information".mysqli_error($connection);
        }
...
<body class="min-h-screen bg-neutral-900 flex items-center justify-center">
    <div class="bg-neutral-800/75 w-full max-w-md sm:max-w-lg lg:max-w-xl p-6 sm:p-8 text-white rounded-lg shadow-lg mx-4">
        ...
        <form action="updateStudent.php" method="post" class="bg-neutral-700/75 p-6 sm:p-8 rounded-lg shadow-md space-y-6">
        <div>
